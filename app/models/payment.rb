class Payment < ActiveRecord::Base
  ...
  def self.sum_today(name, coach, category, transaction_type)
    if !name.nil?
     Payment.where(name: name, :payment_date => (Time.now.beginning_of_day..Time.now.end_of_day), coach_id: coach, category_id: category, transaction_type: transaction_type, :status => PAID).sum(:amount).to_i
   else
